...
table(proba_testowa$recurringaction)
head(proba_uczaca)
```
# Eksploracja danych
...
Nowe zmienne zostały utworzone, ponieważ mogą istonie wpływać na jakość modelu oraz predykcji.
...
Można zauważyć, że tranzakcje inicjalizujące zawierają informację o `browseragent`, a tranzakcje rekurencyjne takiej informacji nie posiadają. W związku z tym imputacja następuje poprzez przypisanie takiej samej wartości zmiennej `browseragent` w tranzakcjach rekurencyjnych jaka jest przy ich tranzakcji inicjalizującej.

Po imputacji braków danych wartości zostają zmienione tak żeby znane było środowisko, w którym działała przeglądarka użyta do realizacji płatności.
...
## Imputacja braków danych w zmiennej `browseragent` w zbiorze `proba_testowa`
`
```{r}
...
W zbiorze danych, niektóre wartości zmiennej `amount` mogą znacznie wpływać na wyniki modelu ze względu na wyjątkowo duże wartości. Zdecydowano żeby odrzucić rekordy ze zmienną `amount` odbiegającą od średniej o 3 odchylenia.
...
## Usunięcie tranzakcji inicjalizujących ze zbioru `proba_uczaca`
...
```
```{r}
# test <- cbind(proba_testowa$id)
# test1 <- cbind(predykcje_testowa$id)
# 
# setdiff(test1,test)
```
